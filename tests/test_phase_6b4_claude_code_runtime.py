"""Tests for Phase 6b4: Claude Code Compatibility Hardening and Public Transcript Contract.
...
class CommittedBootstrapperFilesTests(unittest.TestCase):
    ...
    def test_committed_claude_md_mentions_skills_symlink(self) -> None:
        path = ROOT / ".claude" / "CLAUDE.md"
        content = path.read_text(encoding="utf-8")
        self.assertIn("ln -s", content)
        self.assertIn(".claude/skills", content)

    def test_gitignore_excludes_skills_symlink(self) -> None:
